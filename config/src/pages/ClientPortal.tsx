...
const ClientPortal = () => {
  ...
  const loadAllData = async () => {
    try {
      // Demo mode for testing or username-based access
      if (token === 'demo' || token === 'testtoken' || token?.startsWith('testtoken') || usernameFromSession) {
        loadDemoData();
        setLoading(false);
        return;
      }
  ...
  const loadDemoData = () => {
    // Check if we have a username from session (from login page)
    const sessionUsername = usernameFromSession || 'pppoe_user_001';
    // Demo PPPoE user
    const isPPPoEDemo = !token?.includes('static');

    setClientData({
      user_id: 'demo-user-id',
      username: sessionUsername,
      user_type: isPPPoEDemo ? 'pppoe' : 'static',
      is_active: true,
      phone_number: '+254 700 000 000',
      package_name: isPPPoEDemo ? 'Fiber 100 Mbps' : 'Business Static IP',
      package_price: 2999,
      package_expires_at: new Date(Date.now() + 24 * 60 * 60 * 1000 * 15).toISOString(),
      connection_status: 'connected',
      pppoe_username: isPPPoEDemo ? 'pppoe_user_001@isp' : undefined,
      pppoe_password: isPPPoEDemo ? 'SecurePass123!' : undefined,
      static_ip_address: !isPPPoEDemo ? '196.201.100.50' : undefined,
      static_subnet_mask: !isPPPoEDemo ? '255.255.255.0' : undefined,
      static_gateway: !isPPPoEDemo ? '196.201.100.1' : undefined,
      static_dns_primary: !isPPPoEDemo ? '8.8.8.8' : undefined,
      static_dns_secondary: !isPPPoEDemo ? '8.8.4.4' : undefined,
      static_mac_address: !isPPPoEDemo ? '00:1A:2B:3C:4D:5E' : undefined,
      download_speed_mbps: isPPPoEDemo ? 100 : 500,
      upload_speed_mbps: isPPPoEDemo ? 50 : 250,
      monthly_data_used_mb: 45678,
      last_online_at: new Date().toISOString(),
      current_session_ip: '100.64.123.45',
      session_duration_seconds: 7245,
      bandwidth_limit_mb: 102400, // 100 GB
      bandwidth_used_mb: 45678,
      admin_business_name: 'Demo Fiber ISP',
      admin_contact_phone: '+254 700 000 000',
      admin_contact_email: 'support@demofiber.com'
    });
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const download = Math.floor(Math.random() * 5000) + 1000;
      const upload = Math.floor(Math.random() * 2000) + 500;
      demoUsage.push({
    ...
    setInvoices([
      {
        ...
        pdf_url: '#',
        description: 'Monthly subscription - Fiber 100 Mbps',
    ...
    setNotifications([
      {
        ...
        message: 'We will be performing network maintenance on Sunday 2:00 AM - 4:00 AM. Brief interruptions may occur.',
        notification_type: 'maintenance',
        scheduled_start: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString(),
        scheduled_end: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000 + 2 * 60 * 60 * 1000).toISOString(),
    ...
    setLoginHistory([
      {
        ...
        ip_address: '196.201.100.5',
        login_status: 'success'
      },
      {
        login_at: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(),
        ip_address: '196.201.100.5',
        login_status: 'success'
      },
      {
        login_at: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(),
        ip_address: '196.201.100.10',
        login_status: 'failed',
